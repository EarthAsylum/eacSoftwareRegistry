...
+   `eacSoftwareRegistry_registration.wordpress.trait.php`
+   `eacSoftwareRegistry_registration.includes.php`
+   `eacSoftwareRegistry_registration.interface.php`
+   `eacSoftwareRegistry_registration.interface.trait.php`
+   `eacSoftwareRegistry_registration.refresh.php`
+   `eacSoftwareRegistry_registration.readme.md`


#### Software Registry Implementation

After extracting the `eacSoftwareRegistry_registration.zip` file, move the `eacSoftwareRegistry_registration` folder to your project.

In your project or class file, include `eacSoftwareRegistry_registration/eacSoftwareRegistry_registration.includes.php` to load the required interface and traits.

Your class file must then:

+   implement `\EarthAsylumConsulting\Interfaces\eacSoftwareRegistry_registration`
...
Example:
    ...
    class <your_classname> implements \EarthAsylumConsulting\Interfaces\eacSoftwareRegistry_registration
    {
        use \EarthAsylumConsulting\Traits\eacSoftwareRegistry_registration_wordpress;
            ...
    }

eacSoftwareRegistry_registration.interface.php will have your product and registration server API constants.
...
API parameters are passed as an array:
    ...
    [
        ...
        'registry_product'      => 'productId',                     // * your product name/id (eacSoftwareRegistry)
...
In WordPresss, there are several actions and filters, for file based projects there are corresponding callback methods...
     ...
     filter/callback: eacSoftwareRegistry_api_remote_request( $request, $endpoint )
     ...
     action/callback: eacSoftwareRegistry_api_remote_response( $body, $request, $endpoint )
        @param  array $body response body
        @param  array $request ['method'=>, 'timeout'=>, 'redirection'=>, 'sslverify'=>, 'headers'=>, 'body'=>]
        @param  string $endpoint create, activate, deactivate, verify, refresh, revise

     action/callback: eacSoftwareRegistry_api_remote_$endpoint( $body, $request, $endpoint )
        @param  array $body response body
        @param  array $request ['method'=>, 'timeout'=>, 'redirection'=>, 'sslverify'=>, 'headers'=>, 'body'=>]
        @param  string $endpoint create, activate, deactivate, verify, refresh, revise

     action/callback: eacSoftwareRegistry_api_remote_error( $error, $request, $endpoint )
        @param  object $error {status:{}, error: {}}
        @param  array $request ['method'=>, 'timeout'=>, 'redirection'=>, 'sslverify'=>, 'headers'=>, 'body'=>]
        @param  string $endpoint create, activate, deactivate, verify, refresh, revise

     action/callback: eacSoftwareRegistry_update_registration($registration)
        @param  array $registration registration object

     action/callback: eacSoftwareRegistry_purge_registration()
...
In your class constructor/initialization, add an action to fulfill the registration refresh scheduled by the `scheduleRegistryRefresh()` method in `eacSoftwareRegistry.wordpress.trait.php`

    \add_action( 'eacSoftwareRegistry_registry_refresh', array($this, 'refreshRegistration') );
...
The `addSoftwareRegistryHooks()` method adds:
    ...
    \add_action( 'eacSoftwareRegistry_registry_refresh',     array($this, 'refreshRegistration') );
    \add_filter( 'eacSoftwareRegistry_is_registered',        array($this, 'isValidRegistration') );
    \add_filter( 'eacSoftwareRegistry_registration_key',     array($this, 'getRegistrationKey') );
    \add_filter( 'eacSoftwareRegistry_registration',         array($this, 'getCurrentRegistration') );
    \add_filter( 'eacSoftwareRegistry_registry_value',       array($this, 'isRegistryValue'), 10, 4 );
...
If your primary plugin file name is not the same as the product id, edit your `eacSoftwareRegistry.interface.tpl` file and change the `SOFTWARE_REGISTRY_FILENAME` constant.
...
The `scheduleRegistryRefresh()` method in `eacSoftwareRegistry_registration.filebased.trait.php` is called to schedule the next registration refresh. This method can be modified to schedule a cron event to execute the refresh at a future time. The scheduled event may run `eacSoftwareRegistry_registration.refresh.php <registrationKeyValue>` from the command line to refresh the registration.
